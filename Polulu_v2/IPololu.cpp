...
public:
	...
	unsigned char getPosition(HANDLE port ,unsigned char channel)
	{
		...
		return read;				//response in Byte umrechnung muss noch erfolgen in Postion response = response[0] + 256*response[1] 
	}
